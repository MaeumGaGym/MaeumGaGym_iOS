...
public class SelfCareHomeViewController: BaseViewController<SelfCareHomeViewModel>, Stepper {
    ...
    private let clickProfileRelay = PublishRelay<String>()
    ...
    private let loadProfileRelay = PublishRelay<Void>()
    ...
    var profiles = SelfCareProfileInfoModel(
        nickname: "",
        profileImage: "",
        level: 0,
        wakaTime: 0
    ...
    public override func viewWillAppear(_ animated: Bool) {
        ...
        loadProfileRelay.accept(())

    }
    
    public override func bindViewModel() {
        let input = SelfCareHomeViewModel.Input(
            loadProfile: loadProfileRelay.asDriver(onErrorJustReturn: ()),
        ...
        _ = viewModel.transform(input, action: { output in
            output.profileData.asObservable()
                .subscribe(onNext: { data in
                    self.profiles = data
                    self.nickname = self.profiles.nickname
...
extension SelfCareHomeViewController: UITableViewDataSource {
    ...
    public func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        case .profile:
            self.clickProfileRelay.accept(nickname)
