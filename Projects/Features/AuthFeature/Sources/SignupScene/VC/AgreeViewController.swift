...
public class AgreeViewController: BaseViewController<AgreeViewModel>, Stepper, UIGestureRecognizerDelegate {
    ...
    public override func bindViewModel() {
        ...
        _ = viewModel.transform(input, action: { output in
            ...
            Observable.merge(buttonOutputs)
                .withUnretained(self)
    ...
    public override func bindActions() {
        ...
        firstSeemoreButton.rx.tap.subscribe(onNext: { [weak self] _ in
            self?.openURL("https://info-dsm.notion.site/e9d45a0490674b81a419bbc4cbdd5a9d?pvs=4")
        }).disposed(by: disposeBag)
        
        secondSeemoreButton.rx.tap.subscribe(onNext: { [weak self] _ in
            self?.openURL("https://info-dsm.notion.site/2a0474e87f754fbe8f53d58f2003ccb2?pvs=4")
        }).disposed(by: disposeBag)
    }
    ...
    private func openURL(_ urlString: String) {
        if let url = URL(string: urlString) {
            ...
            safariView.modalPresentationStyle = .formSheet
    ...
    public func setAllAgreeButtonState(_ isEnabled: Bool) {
        [
            ...
            isEnabled ? $0.buttonYesChecked() : $0.buttonNoChecked()
        }
        updateAllAgreeButtonState()
    }
    
    public func updateAllAgreeButtonState() {
        if allAgreeButtonState {
    ...
    public func buttonActivationChecked(button: MGCheckButton) -> Bool {
        let allRequiredButtonsChecked = firstAgreeButton.checked &&
                                        secondAgreeButton.checked &&
                                        thirdAgreeButton.checked
        let shouldActivateButton = allRequiredButtonsChecked && (fourthAgreeButton.checked || !fourthAgreeButton.checked)
...
private extension AgreeViewController {
